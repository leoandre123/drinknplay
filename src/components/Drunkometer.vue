<template>
  <svg view-box="0 0 100 100" fill="none">
    ...
    <path
      ...
      A 35 35 0 0 1 ${50 + Math.cos(l.end) * 35} ${50 + Math.sin(l.end) * 35}`"
      :stroke="l.color"
      stroke-width="30"
    />
    ...
    <path
      v-for="lineAngle in majorLines"
      :d="`M ${50 + Math.cos(lineAngle) * 40} ${50 + Math.sin(lineAngle) * 40}, L ${
        50 + Math.cos(lineAngle) * 50
      } ${50 + Math.sin(lineAngle) * 50}`"
      stroke="black"
      stroke-width="2"
    />
    <path
      v-for="lineAngle in minorLines"
      :d="`M ${50 + Math.cos(lineAngle) * 45} ${50 + Math.sin(lineAngle) * 45}, L ${
        50 + Math.cos(lineAngle) * 50
      } ${50 + Math.sin(lineAngle) * 50}`"
      stroke="black"
      stroke-width="1"
    />
    <path
      :d="`M ${50 + Math.cos(angle) * 10} ${50 + Math.sin(angle) * 10}, L ${
